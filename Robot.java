/*
 * Main class to run Robot Animation
 *
 * @author Mahesha Kulatunga mahesha.kulatunga@gmail.com
...
public class Robot extends Frame implements GLEventListener, ActionListener,
  ...
  private static final float FAR_CLIP=500.0f;
  ...
  public Robot() {
    ...
    Panel p = new Panel(new GridLayout(2,1));
      Panel p1 = new Panel(new GridLayout(5,1));
        checkBackgroundLight = addCheckbox(p1, "Background lights", this);
        checkHeadLamp = addCheckbox(p1, "Head lamp", this);
      p.add(p1);
  ...
  public void itemStateChanged(ItemEvent e) {
    ...
    if (source == checkBackgroundLight) {
      scene.getBGLight1().setSwitchedOn(checkBackgroundLight.getState());
      scene.getBGLight2().setSwitchedOn(checkBackgroundLight.getState());
      canvas.repaint();
    }
    if (source == checkHeadLamp) {
      scene.getHL().setSwitchedOn(checkHeadLamp.getState());
      canvas.repaint();
    }
  }
  ...
  private void reset() {
    checkBackgroundLight.setState(true);
    scene.getBGLight1().setSwitchedOn(true);
    scene.getBGLight2().setSwitchedOn(true);
 ...
 /*
  ...
  public void init (GLAutoDrawable drawable) {
    ...
    double radius = RADIUS;           // radius of 'camera sphere', i.e. distance from
	                                  // world origin
    ...
    scene = new RobotSetScene(gl, camera);
  }
